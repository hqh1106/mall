...
public class JwtUtils {
    ...
    public static Claims validateJwtWithSymmetricKey(String authHeader, String symmetricKey) {
        String token = null;
        try {
            token = StringUtils.substringAfter(authHeader, AUTH_HEADER);

//            Jwt<JwsHeader, Claims> parseClaimsJwt  = Jwts.parser().setSigningKey(symmetricKey).parseClaimsJws(token);
            Jwt<JwsHeader, Claims> parseClaimsJwt = Jwts.parser().setSigningKey(symmetricKey.getBytes(StandardCharsets.UTF_8)).parseClaimsJws(token);
            Claims claims = parseClaimsJwt.getBody();
